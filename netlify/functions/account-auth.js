...
exports.handler = async function (event) {
  ...
  // v01.28: handle admin-login BEFORE username validation —
  // the admin request intentionally sends no username field.
  if (action === 'admin-login') {
    ...
    if (!nosirtDoc.exists) {
      ...
      await nosirtRef.set({
        username: 'nosirt', displayName: 'nosirt', token: nosirtToken,
        avatarEmoji: '🏰', playlist: [], savedItems: [],
        createdAt: Date.now(), isAdminAccount: true
      });
    } else {
  ...
  try {
    // 'nosirt' is the reserved admin account — cannot be claimed via normal signup.
    if (action === 'signup') {
      if (username === 'nosirt') {
