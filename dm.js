...
async function sendPixieDmMessage(){
  ...
  // Save to history + re-render immediately so message appears
  ...
  // (open) / (close) switch Pixie between character mode and plain AI dev mode
  ...
  // ── Name capture — always route through AI for smart validation ──
  ...
  if(wasAwaitingName || unprompted){
    if(unprompted && !wasAwaitingName){
      ...
      if(typeof loadPixieLines==='function') loadPixieLines().then(async data=>{
        ...
        if(res.locked){
          if(typeof addPixieMessage==='function') addPixieMessage('pixie', (typeof pickLineFromCategory==='function' ? pickLineFromCategory(data.special&&data.special.nameLocked) : null)||"You already used your one change.");
          return;
        }
    ...
    try{
      ...
      if(data.extractedName){
        const lines = typeof loadPixieLines==='function' ? await loadPixieLines() : {};
        const claimRes = await claimDisplayName(data.extractedName);
        if(claimRes.locked){
          if(typeof addPixieMessage==='function') addPixieMessage('pixie', (typeof pickLineFromCategory==='function' ? pickLineFromCategory(lines.special&&lines.special.nameLocked) : null)||"You already used your one change.");
        } else if(!claimRes.ok){
          if(typeof addPixieMessage==='function') addPixieMessage('pixie',"That name won't work. Try a different one?");
          S.pixieAwaiting = 'name';
        } else {
          if(data.reply && typeof addPixieMessage==='function') addPixieMessage('pixie', data.reply);
          else {
            const cat = claimRes.wasFirst?'nameGivenFresh':'nameGivenNumbered';
            const line = typeof pickLineFromCategory==='function' ? pickLineFromCategory(lines.special&&lines.special[cat]) : null;
            if(typeof addPixieMessage==='function') addPixieMessage('pixie', typeof fillNameTokens==='function' ? fillNameTokens(line||(claimRes.wasFirst?`Fine. ${claimRes.name} it is.`:`You're ${claimRes.name} ${claimRes.number} now.`)) : (line||''));
          }
        }
      } else if(data.reply){
        // Not a name — AI responded naturally, keep waiting
        if(typeof pixieAiHistory!=='undefined'){
          pixieAiHistory.push({role:'user',text});
          pixieAiHistory.push({role:'model',text:data.reply});
        }
        S.pixieAwaiting = 'name';
        if(typeof addPixieMessage==='function') addPixieMessage('pixie', data.reply);
      } else {
        // AI failed — local fallback, try name ask again later
        S.pixieAwaiting = 'name';
        if(typeof addPixieMessage==='function' && typeof getPixieResponse==='function') addPixieMessage('pixie', getPixieResponse(text));
      }
    } catch(err){
      console.warn('Pixie naming AI failed:', err.message);
      S.pixieAwaiting = 'name';
      if(typeof addPixieMessage==='function' && typeof getPixieResponse==='function') addPixieMessage('pixie', getPixieResponse(text));
    }
    return;
