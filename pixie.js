...
function updatePixieIconVisibility(){
  ...
  if(!icon) return;
  // "Map" is active when page-map is visible (no other page is shown)
  const mapPage=$('page-map')||document.getElementById('map-viewport');
  const isMap = mapPage ? (mapPage.style.display!=='none' && !mapPage.classList.contains('hidden')) : true;
  ...
  try {
    ...
    if (data.reply) {
      ...
      setTimeout(() => {
        ...
        // Mark tip as given if this reply seems like a tip (heuristic)
