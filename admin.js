...
// ── Claim all existing shows as nosirt account (one-time migration) ──
async function claimAllShowsAsNosirt(){
  if(!S.adminUnlocked){ toast('admin only'); return; }
  const shows = S.shows || [];
  if(!shows.length){ toast('no shows found'); return; }
  const db = firebase.firestore();
  const batch = db.batch();
  let count = 0;
  shows.forEach(s=>{
    if(!s.owner || s.owner==='admin'){
      batch.update(db.collection('nosirt_shows').doc(s.id),{ owner:'nosirt', isPublic: s.isPublic||false });
      count++;
    }
  });
  if(!count){ toast('all shows already owned by nosirt ✓'); return; }
  try{
    await batch.commit();
    toast(`${count} show${count===1?'':'s'} claimed as nosirt ✓`);
  } catch(e){
    toast('claim failed: '+e.message);
    console.error('claimAllShowsAsNosirt:',e);
  }
}
...
function renderAdminShowsTab(){
  const el=$('admin-tab-shows');
  if(!el)return;
  const total = (S.shows||[]).length;
  const unowned = (S.shows||[]).filter(s=>!s.owner||s.owner==='admin').length;
  el.innerHTML=`
    <div class="admin-section-title" style="margin-bottom:10px">show ownership</div>
    <div style="font-family:'IM Fell English',serif;font-size:.78rem;color:var(--fog);margin-bottom:10px">
      ${total} shows total. ${unowned} not yet assigned to nosirt.
    </div>
    ${unowned>0?`<button class="sq-submit" style="width:100%;font-size:.75rem;margin-bottom:4px" onclick="claimAllShowsAsNosirt()">
      claim all ${unowned} unowned show${unowned===1?'':'s'} as nosirt →
    </button>`:'<div style="color:#8fc97a;font-size:.75rem">✓ all shows assigned to nosirt</div>'}
    <div style="font-size:.65rem;color:var(--fog);opacity:.5;margin-top:6px;font-style:italic">
      After claiming, shows appear in your library dropdown and can be made public or private.
    </div>
  `;
}

function switchAdminTab(tab){
  ['features','display','chat','environment','pixie','users','moderation','shows'].forEach(t=>{
  ...
  if(tab==='pixie' && typeof renderPixiePersonalityAdmin==='function') renderPixiePersonalityAdmin();
}
...
async function handleAdminLoginProfile() {
  ...
  // Notify Pixie of admin login (switches to admin history store)
  if(typeof window.pixieOnAdminLogin === 'function') window.pixieOnAdminLogin();
...
function handleAdminLogoutProfile() {
  // Notify Pixie of admin logout (restores regular history)
  if(typeof window.pixieOnAdminLogout === 'function') window.pixieOnAdminLogout();
